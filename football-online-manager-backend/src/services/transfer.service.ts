...
export class TransferService {
  ...
  async addPlayerToTransferList(
    ...
    const availablePlayers = team.players.filter((p) => {
      const hasActiveTransfer = p.transfers.some(
    ...
    logger.info(
      `Player ${player.name} added to transfer list by team ${player.team.name}`
    );
  ...
  async removePlayerFromTransferList(
    ...
    logger.info(
      `Transfer cancelled for player ${transfer.player.name} by team ${transfer.sellingTeam.name}`
    );
  }

  async buyPlayer(
    buyingTeamId: string,
    buyData: BuyPlayerDto
  ): Promise<TransferResponse> {
    const transfer = await this.transferRepository.findOne({
    ...
    try {
      ...
      logger.info(
        `Player ${transfer.player.name} sold from ${transfer.sellingTeam.name} to ${buyingTeam.name} for ${purchasePrice}`
      );
  ...
  async getTeamTransfers(teamId: string): Promise<TransferResponse[]> {
    const transfers = await this.transferRepository.find({
      where: [{ sellingTeam: { id: teamId } }, { buyingTeam: { id: teamId } }],
      relations: ["player", "sellingTeam", "buyingTeam"],
